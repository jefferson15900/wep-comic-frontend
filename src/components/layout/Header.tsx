...
const Header = () => {
  ...
  const handleMobileMenuClick = () => {
    ...
    // Y al mismo tiempo, se asegura de que el panel de notificaciones esté cerrado.
        ...
        <div className={`flex-1 flex justify-between items-center transition-opacity duration-300 ${isSearchOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
          ...
          <div className="relative flex items-center gap-2">
            ...
            <div className="flex lg:hidden items-center gap-1">
              ...
              {user && (
                  <button onClick={handleBellClick} className="p-2 text-gray-300 hover:text-white rounded-full relative" title="Notificaciones">
                    <Bell size={22} />
                    {unreadCount > 0 && <span className="absolute top-1.5 right-1.5 flex h-2 w-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-red-500"></span></span>}
                  </button>
              )}
              <button onClick={handleMobileMenuClick} className="p-2 text-gray-300 hover:text-white rounded-full" aria-label="Abrir menú">
                <Menu size={24} />
              </button>
            </div>

            {isNotificationsOpen && (
                <div ref={notificationsRef} className="absolute right-0 top-full mt-2 w-80 bg-[var(--surface-dark)] rounded-lg shadow-xl z-40 border border-gray-700">
